...
def initializeDates(returnCase: str):
    ...
    end_time = time - timedelta(
        seconds=15
    )  # Si sottraggono 15 secondi onde evitare: https://stackoverflow.com/questions/66728533/twitter-api-v2-response-failing-when-params-has-start-time-end-time
    start_time, end_time = start_time.strftime(dtformat), end_time.strftime(dtformat)
    match returnCase:
        case "HTMLFormat":
            start_time, end_time = (
                start_time[0 : len(start_time) - 4],
                end_time[0 : len(end_time) - 4],
            )
            return {
                "minDate": start_time,
                "minDateValue": start_time,
                "maxDate": end_time,
                "maxDateValue": end_time,
            }
        case "APIFormat":
            return {
                "start_time": start_time[0:17] + "00Z",
                "end_time": end_time[0:17] + "00Z",
            }
        case _:
            # TODO
            pass
